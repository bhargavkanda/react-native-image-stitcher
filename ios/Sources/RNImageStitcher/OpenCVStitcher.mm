...
+ (NSDictionary<NSString *, NSNumber *> *)applyOutputControlsAtPath:(NSString *)imagePath
  ...
  std::string nativePath(cleaned.UTF8String);
  // iOS cv::imread honours the EXIF orientation tag, so the decoded
  // pixels already match what the source displayed.
  cv::Mat img = cv::imread(nativePath, cv::IMREAD_COLOR);
  if (img.empty()) {
    if (error) {
      *error = [NSError errorWithDomain:RNImageStitcherErrorDomain
  ...
  // Downscale to fit (aspect preserved). A cap <= 0 is unbounded on
  // that axis; scale >= 1 means no resize (still re-encoded at quality).
  double scale = 1.0;
  if (maxWidth > 0) {
    double sw = (double)maxWidth / (double)img.cols;
    if (sw < scale) { scale = sw; }
  }
  if (maxHeight > 0) {
    double sh = (double)maxHeight / (double)img.rows;
    if (sh < scale) { scale = sh; }
  }
  cv::Mat out;
  if (scale < 1.0) {
    int newW = (int)((double)img.cols * scale);
    int newH = (int)((double)img.rows * scale);
    if (newW < 1) { newW = 1; }
    if (newH < 1) { newH = 1; }
  ...
  bool ok = cv::imwrite(nativePath, out, writeParams);
  if (!ok) {
    if (error) {
      *error = [NSError errorWithDomain:RNImageStitcherErrorDomain
                                   code:1022
  ...
  return @{
    @"width":  @((NSInteger)out.cols),
    @"height": @((NSInteger)out.rows),
  };
}
