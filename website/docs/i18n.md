...
For a *finalized* lateral stop the popup and the crop warning banner are
alternatives rather than both — with a review surface configured the banner is
what the operator sees, so translate `warnLateralDriftFinalize` with the same
care as the popup keys:
...
| `lateralStopTitle` | lateral-drift popup (capture kept + stitched) — only when no review surface follows ([why](./camera-api.md#what-shows-after-a-lateral-drift-stop)) | `Keep the pan straight` |
| `lateralStopBody` | lateral-drift popup (capture kept + stitched) — only when no review surface follows ([why](./camera-api.md#what-shows-after-a-lateral-drift-stop)) | `You moved sideways. Pan in one direction only — we stitched what you captured.` |
