...
export function selectCaptureDevice<D extends DeviceLike>(
  ...
  // is unaffected either way (see the branch below -- the 1× primary pick
  // still mounts this same multicam device, so torch/format continuity at
  // 1× does not change; only the 0.5× target does). iOS is untouched:
  // this whole gate is a no-op unless platform === 'android'.
